...
(
    ...
    // 9600*16 = 153, 600
    // 100_000_000 Hz / 153_000 = 651ns(샘플링)
    parameter integer DIVIDER_CNT=100_100_000/(BPS*16);

    reg [1:0] r_state; //state 상태 저장
    reg [3:0] r_bit_cnt; // r_data_reg에 저장할 index값
    reg [7:0] r_data_reg; // rx포트로부터 들어온 bit를 담음
    reg [15:0] r_baud_cnt; // 651ns =9600bps*16(sampling 구간)
    reg r_baud_tick; // 651ns마다 tick발생
    reg [3:0]  r_baud_tick_cnt; // 16개 오버샘플링 cnt
